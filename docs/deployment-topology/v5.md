...
Data sources: this repo's compose files / k8s manifests / tofu / tailscale policy / READMEs + live inspection of every host (`docker ps`, `docker network ls`, `free`, `uptime`, `df`, `tailscale ip`, `systemctl`, `kubectl get nodes/ns/pods/svc/pv/resourcequota/clusterpolicy/sealedsecret/vulnerabilityreport`, `kubectl top nodes`, `docker exec npm` reading all 25 `/data/nginx/proxy_host/*.conf`, Prometheus `/api/v1/targets`). Full list at the end.
...
| Runs | 13 compose stacks running (14 defined) + 1 compose project outside the repo, k3s **server** node, 3 host-native services | 4 compose stacks, k3s **agent** node | 4 compose stacks |
...
flowchart TB
    ...
    subgraph ORACLE["vps_oracle · OCI tenancy 1 · arm64 4C/23G · the hub, where this repo runs"]
        direction TB
        onpm["npm<br/>only 80/443 entry, 25 proxy records"]
...
## NPM reverse-proxy records (25, live)
...
| docker container that does not exist | `ollama` → `open-webui:8080` | **502: `llm` stack not running** |
...
- **`llm` stack is defined but not running.** `vps_oracle/compose/llm/` (llama-cpp + open-webui) exists in the repo, but neither container exists on the host, and NPM record `ollama.*` returns 502. Its images are still present locally.
