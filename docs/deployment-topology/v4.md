...
6. Several things v3 never drew are now included because they exist: the shared data services (`minio` / `postgres` / `redis` + their UIs), the `love-bird-boss-dev` dev environment, and two stray test containers.
...
flowchart TB
    ...
    subgraph ORACLE["vps_oracle · OCI tenancy 1 · arm64 4C/23G · the hub, where this repo runs"]
        direction TB
        onpm["npm<br/>only 80/443 entry, 29 proxy records"]
    ...
    onpm -."tailscale, gcp-verify.dev.*".-> gverify
    ...
    class ok3s k8s
    class o2dify,o2ne,o2gl,o2pa,o2tofu,gverify,gne,gpa,gtofu spoke
...
flowchart TB
    ...
    subgraph HOST["vps_oracle host"]
        ...
        subgraph ProxyNet["docker network: proxy (172.19.0.0/16, external bridge, 19 members)"]
            direction TB
    ...
    class Internet,VlessClient entry
    class npm,xui,portainer,grafana,ccr,switchboard,plans,homepage,trilium,vikunja,vjrelay,apprise,minio,pgadmin,redisinsight,prometheus,bossfe,bossbe,bossbk proxynet
...
|---|---|---|
| docker `proxy` network, by container name | `portainer`, `grafana`, `trilium`, `vikunja`, `apprise`, `homepage`, `switchboard`, `ccr`, `plans`, `minio`, `pgadmin`, `redisinsight`, `boss.dev` | live |
...
| **vps_gcp** over tailscale | `gcp-verify.dev` → `100.96.184.44:8080` | live |
...
Now 19 members. Shrank by the 9 dify containers (moved to oracle2) and by `evidence-os-website` and `cc-window` (both removed 2026-09-19); grew with `minio`, `pgadmin`, `redisinsight` and the three `love-bird-boss-dev` containers, which v3 did not draw. Static IPs are unchanged: `3x-ui` `.2`, `npm` `.3`, `prometheus` `.4`.
...
### vps_gcp
A deliberately minimal practice node: `gcp-verify` (nginx smoke test), `node-exporter`, `portainer-agent`. With ~1 GiB and no swap it has ~490 MiB available; it is not a place to add real workloads. Its IP is ephemeral (changes on `tofu destroy`/`apply`); the tailscale IP is what the rest of the fleet uses.
...
- **NodePort relays 6 → 11**; **NPM records 24 → 29**.
