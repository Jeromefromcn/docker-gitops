...
- **Spec coverage**: Trivy Operator (Job-only) → Task 2; Kyverno (admission-controller only) → Task 3; Cosign signature verification (self-built image scope) → Task 4; Trivy CVE gate (cluster-wide) → Task 5; restricted-equivalent (per-workload) → Task 6; PSA baseline (8 namespaces) → Task 7; Audit→Enforce cadence → Task 8/9; securityContext fixes (placeholder-hello base image swap, containerPort change, vikunja-notify-relay declaration added) → Task 1; verification checklist items 9/10 (deliberate-violation test after Enforce + full Application re-check) → Task 9 Step 3-5.
- **Placeholder scan**: Task 5's policy is explicitly marked "first-draft, needs adjusting against the live schema tested in Task 2 Step 6" — this isn't a TBD, it's making the necessary empirical step of "verify against the live CRD" an explicit part of the task, with concrete starting-version content already given, not left blank for the implementer to figure out. No other task has a TBD/TODO.
- **Type consistency**: the `app in (placeholder-hello, vikunja-notify-relay)` selector is consistent between the Task 6 policy and the Task 1 Deployment label; the three policy filenames (`restrict-image-registry.yaml`/`require-vuln-scan-clean.yaml`/`restricted-self-built.yaml`) stay consistent from Task 3's README description, through creation in Task 4-6, to flipping to Enforce in Task 9; version numbers such as `--controller-name`/chart version stay consistent throughout Task 1, sourced from actual testing of `helm search repo`/GitHub Release on 2026-08-15, not from memory.
