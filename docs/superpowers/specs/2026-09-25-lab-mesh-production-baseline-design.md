...
A fourth trap, found while closing out the acceptance evidence: **the hop that reports the timeout is not always the hop that is at fault.** During `visits_redis_timeout`, `/api/customer/owners/{id}/visits` makes customers-service wait on visits-service, so the waypoint's 1s `perTryTimeout` on the *customers* route fires and the symptom is logged as `504 UT` against `customers-service:8081` — pointing an investigator one hop short of the real cause. Measured: `UT` against `visits-service:8082` at 17:17:00–17:17:22Z, and against `customers-service:8081` at 17:17:15–17:17:19Z within the same scenario. Contrast `customers_slow_query`, whose `UT` against `customers-service:8081` (17:15:23–17:15:47Z) *is* the fault's own hop. A runbook consequence: each scenario run must record its own start/end timestamps. The first attempt to attribute these windows read a 6-minute aggregate of the traffic generator's log and blamed the wrong scenario entirely — the per-scenario Loki time windows above are what settled it.
