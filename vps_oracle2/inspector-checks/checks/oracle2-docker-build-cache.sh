#!/usr/bin/env bash
# vps_oracle2/inspector-checks/checks/oracle2-docker-build-cache.sh
#
# vps-oracle2 counterpart of checks/docker-build-cache.sh (same logic and tier), run
...
# of the cache, so plain `docker builder prune` silently reclaimed nothing every run
# while still reporting "deleted". --all only costs a slower next build.
set -uo pipefail
...
require_daemon "oracle2-docker-build-cache.sh"
