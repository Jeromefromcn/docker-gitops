# k3s/install/agent-vps-oracle2

vps-oracle2 is a **k3s agent (worker) node** of the cluster whose server runs on vps_oracle ([`k3s/`](../../README.md)). The control plane and every management component stay on vps_oracle; this node only runs workloads that explicitly tolerate its taint (lab-environment), plus the per-node DaemonSets (cilium, cilium-envoy, istio-cni, ztunnel).

Nothing here is cloned onto oracle2 (the repo lives only on vps_oracle). [`install.sh`](install.sh) runs **on vps_oracle** and drives oracle2 over SSH.
...
```bash
k3s/install/agent-vps-oracle2/install.sh    # on vps_oracle
```
...
- **Tailscale addressing.** The two hosts are in separate OCI tenancies; oracle2 cannot reach vps_oracle's VCN address `10.0.0.95`. Both nodes use their tailscale IP as node IP (the server's side is in `k3s/install/config.yaml`), so Cilium's VXLAN tunnel runs inside tailscale's WireGuard. Pod MTU is 1230 (tailscale0 1280 − 50 VXLAN), auto-detected by Cilium.
...
(e.g. after `tofu destroy`/`apply`, see [`vps_oracle2/README.md`](../../../vps_oracle2/README.md)) — update `node-ip` here and rerun `install.sh`; also delete the stale `Node` object (`kubectl delete node <old>`) if the hostname changed.
