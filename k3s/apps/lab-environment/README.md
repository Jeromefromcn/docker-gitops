...
×2 from their `parametersRef` ConfigMaps). All four business services use
`RollingUpdate` with `maxSurge: 1 / maxUnavailable: 0`, so a rollout never
drops below the replica count.
...
an oversight, but it has a consequence worth knowing before demonstrating
node-drain safety: with a single replica and no PDB, draining vps-oracle2
evicts vets-service outright, and the `Lab API Down` probe goes to
`/api/vet/vets`. A drain demo must either start vets/visits at ≥2 replicas
first or expect that alert to fire.

A `lab.jerome/rollout-rev` annotation on `customers-service` is the repeatable
...
sequences never rewind over rows the apps inserted).

The ordering is the subtle part: hook pods run *before* the same sync applies
regular resources, so a plain ServiceAccount/ConfigMap would not exist yet and
the Job would fail to create. The ServiceAccount and the SQL ConfigMap are
therefore themselves PreSync hooks at `sync-wave: "-1"`, which orders them
*before* the Job (itself `PreSync`, at the default wave 0). **Do not "tidy"
the Job to `-1` as well** — equal waves mean no ordering between them, which is
exactly the failed-hook state this arrangement exists to fix.
