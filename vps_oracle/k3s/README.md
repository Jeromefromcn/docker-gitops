...
- **`homepage` (gethomepage/homepage:v1.13.2) and `trilium` (triliumnext/trilium:v0.104.1): CVE-2026-59873, fixed in `7.5.19`.** Both correctly denied by the policy. Not yet resolved — need to check whether a newer image tag for either service picks up the fix, or add a documented time-boxed exception if not. **Blocks nothing today** (Audit mode), but this is the actual reason `Enforce` hasn't been flipped on yet for either policy.
- **Cosign verification fails inside Kyverno for both self-built images, despite the signatures being genuinely valid.** `cosign verify` from the CLI succeeds against the exact same digests (confirmed for `placeholder-hello@sha256:56fdf254...`); Kyverno's built-in Sigstore client reports `no signatures found` for the identical digest with no lower-level error in its logs. Root cause not yet identified — leading theory is a TUF trust root or Rekor endpoint mismatch between Kyverno's bundled Sigstore client and the `cosign` CLI's default config, but this needs dedicated investigation (checking Kyverno's `--tufRoot`/`--tufMirror` install flags, and whether the `kyverno` namespace has the egress it needs to reach Fulcio/Rekor) before `restrict-image-registry` can safely go to `Enforce` — as written today, flipping it would permanently block every deployment of every self-built image, since neither one currently passes.
- **`vikunja-notify-relay`'s CI workflow (`vikunja-notify-relay.yml`) fails on `workflow_dispatch` with `permission_denied: write_package`** pushing to GHCR — this appears to be the first time this workflow has ever actually run to completion since it was created in phase D (the original image was pushed manually, not via this pipeline). Needs the GHCR package's "Manage Actions access" settings checked/granted for this repo (GitHub web UI, or `gh auth refresh` with `write:packages` scope) before a fresh signed build can be produced. Until fixed, `vikunja-notify-relay`'s Deployment stays on its `:1.1.0` tag (unpinned to digest) rather than pinning to an unverifiable digest.
...
**Net effect: `Enforce` has not been flipped for any of the three policies.** The Cosign verification issue in particular is a hard blocker — it would break deployment of every self-built image outright. `require-vuln-scan-clean` and `restricted-self-built` are individually closer to Enforce-ready (their remaining findings are either accepted risk or a specific known CVE with a clear resolution path), but flipping policies independently rather than all at once hasn't been evaluated yet.
