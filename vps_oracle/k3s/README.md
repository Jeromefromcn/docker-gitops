...
- `require-vuln-scan-clean` — denies Pods with a `VulnerabilityReport` showing a `CRITICAL` vulnerability that **has a fix available** (`fixedVersion != ''`), cluster-wide, not scoped to self-built images. Deliberately does NOT deny on unfixable CRITICALs — blocking those has no resolution path and would just deadlock the workload forever
...
Adding a new self-built workload to the restricted policy: edit the `values` list in `restricted-self-built.yaml`'s label selector.
...
- **CVE-2026-59873 (`node-tar` DoS via crafted gzip bomb) — homepage and trilium RESOLVED (2026-08-16), sillytavern remains a documented accepted exception (re-check by 2026-11-15).** The vulnerable tar lives in the **npm CLI's internal bundled copy** (`/usr/local/lib/node_modules/npm/node_modules/tar`, version follows the node base image, not the app), so no upstream release of any of these apps fixes it. homepage (`tar 7.5.11`) and trilium (`tar 7.5.15`) were rebuilt as **security wrapper images** (`vps_oracle/k3s/images/`, built+signed by the `patched-images` workflow, cosign v3.1.3) that replace npm with 11.19.0 (bundles tar `7.5.19`) — the patched images are Trivy-clean (CI gate: 0 CRITICAL). homepage also moved `v1.13.2 -> v2.0.0` in the process: v1.13.2 additionally carried tar `7.5.11` as an app dependency (pnpm), which a wrapper can't remove; v2.0.0 dropped it (config verified compatible via a live smoke test). **sillytavern** (`tar 7.5.11`) stays an exception: it also carries `protobufjs 6.11.4` with fixable CRITICAL CVE-2026-41242 (arbitrary code execution, pulled in via `sillytavern-transformers`/`onnx-proto`), which a wrapper can't fix and a protobufjs 6→7/8 override would risk breaking — needs the upstream transformers.js fork to bump it. Neither remaining issue blocks anything today (Audit mode), but flipping `require-vuln-scan-clean` to `Enforce` would deny sillytavern rollouts until then.
- **Known limitation: the cluster-side trivy-operator only scans amd64-compatible image variants** ("no child with platform linux/amd64" on single-arch arm64 manifests) — our self-built arm64 images (`placeholder-hello`, `vikunja-notify-relay`, and the patched `homepage`/`trilium`) get no in-cluster `VulnerabilityReport`, so `require-vuln-scan-clean` passes them by absence of a report. The CI `build-scan-sign`/`patched-images` Trivy gates (exit-code 1 on fixable CRITICAL) are the actual protection for those; multi-arch official images (apprise, etc.) still get in-cluster scans. Fixing operator-side arm64 scanning is a separate task (no obvious config in trivy-operator v0.33.0).
...
- **Pre-existing quota headroom issues surfaced (not caused) by restart-testing these policies**, in namespaces unrelated to phase E's own additions: `dify-quota`'s `limits.cpu` was already at 2850m/3000m and `workloads-quota` at 1700m/2000m before any of this work started — restarting `dify`'s app tier (`api`/`worker`/`worker-beat`/`web`) and `trilium` each hit the same `RollingUpdate`-vs-tight-quota deadlock already fixed for `llm` via `Recreate` strategy (commit `bd79748`). Not yet applied to `dify` or `workloads`. Follow-up, not a phase E regression.

**Net effect: `Enforce` has not been flipped for any of the three policies.** The Cosign blocker is resolved; the CVE-2026-59873 finding is resolved for homepage and trilium via the patched images, leaving only **sillytavern** (protobufjs CVE-2026-41242) as a `require-vuln-scan-clean` deny under Enforce. `Enforce` now waits on: deciding whether to exclude sillytavern (`skipImageReferences`) or wait for its upstream dependency bump, and the deliberate multi-day Audit observation window. `restricted-self-built` and `restrict-image-registry` are Enforce-ready for all self-built images (placeholder-hello, vikunja-notify-relay, and now the patched homepage/trilium, all verified passing).
