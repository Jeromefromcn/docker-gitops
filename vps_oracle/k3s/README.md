...
**Net effect: `Enforce` has not been flipped for any of the three policies.** The Cosign blocker is resolved for `placeholder-hello`, but `Enforce` still waits on: a fresh v3.1.3-signed `vikunja-notify-relay` build (GHCR permission fix first), a decision on the `homepage`/`trilium` CVE-2026-59873 finding, and the deliberate multi-day Audit observation window. `require-vuln-scan-clean` and `restricted-self-built` are individually closer to Enforce-ready (their remaining findings are either accepted risk or a specific known CVE with a clear resolution path), but flipping policies independently rather than all at once hasn't been evaluated yet.
