...
- **CVE-2026-59873 (`node-tar` DoS via crafted gzip bomb) — documented accepted exception (2026-08-16), re-check by 2026-11-15.** Affects three images, all checked for a fixed upstream tag and **none has one**: `homepage` (`gethomepage/homepage:v1.13.2`, tar `7.5.11`), `trilium` (`triliumnext/trilium:v0.104.1`, tar `7.5.15`), `sillytavern` (`ghcr.io/sillytavern/sillytavern:1.18.0`, tar `7.5.11`). `trilium` and `sillytavern` are already at their latest published versions. For `homepage`, the newest release `v2.0.0` was pull-tested and *still* reports the CVE — the vulnerable tar is the **npm CLI's internal bundled copy** (`/usr/local/lib/node_modules/npm/node_modules/tar`, version follows the node base image, not the app), and Trivy scans it, so no app-version bump can clear it. All three are third-party published images we consume without rebuilding, so there's no fix path from this side; the affected tar is realistically unreachable from each app's runtime surface (npm CLI runs at image build time), which is why this is an accepted time-boxed exception rather than a block. **Blocks nothing today** (Audit mode), but flipping `require-vuln-scan-clean` to `Enforce` would deny rollouts of all three — if Enforce is wanted before the re-check, those three images need excluding from the policy (`skipImageReferences`), not just the exception note.
...
**Net effect: `Enforce` has not been flipped for any of the three policies.** The Cosign blocker is resolved for both self-built images; the `homepage`/`trilium`/`sillytavern` CVE-2026-59873 finding is recorded as a documented accepted exception (no upstream fix exists, see the finding above); `Enforce` now waits on: deciding whether to exclude those three images from `require-vuln-scan-clean` (`skipImageReferences`) or keep waiting out the exception re-check, and the deliberate multi-day Audit observation window. `restricted-self-built` is independently Enforce-ready (both self-built images verified passing).
