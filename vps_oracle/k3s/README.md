...
**Net effect: `Enforce` has not been flipped for any of the three policies.** The Cosign blocker is resolved; the CVE-2026-59873 finding is resolved for homepage and trilium via the patched images, and **sillytavern** (the last `require-vuln-scan-clean` deny) was deleted on 2026-08-16 — no known fixable-CRITICAL image remains in scope. `Enforce` now waits on: the deliberate multi-day Audit observation window, and deciding whether `require-vuln-scan-clean` is Enforce-ready given the trivy-operator's arm64 scan gap (self-built images pass by absence of an in-cluster report; CI gates are their real protection). `restricted-self-built` and `restrict-image-registry` are Enforce-ready for all self-built images (placeholder-hello, vikunja-notify-relay, and now the patched homepage/trilium, all verified passing).
