...
# missing logging.options.max-size. Design spec's "compose logging
  ...
  while read -r svc; do
    ...
    if [ -z "$max_size" ]; then
      emit_result "alert" "flagged" "$relpath:$svc" \
        "service has no logging.options.max-size (repo convention: max-size 10m, see root README log-size-limit section)"
    fi
