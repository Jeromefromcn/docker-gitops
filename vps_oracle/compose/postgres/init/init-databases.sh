...
# either re-run on a fresh data dir OR apply it manually (see README "How to add a new app").
#
...
create_role_and_db() {
  ...
  if ! "${psql[@]}" -tAc "SELECT 1 FROM pg_roles WHERE rolname='${app}'" | grep -q 1; then
    # Password is a placeholder; the app's real connection uses a stronger one
    # set by whoever provisions the app (see README). Kept here so the DB exists
