...
# declaring 1M removes the limit — same provenance as on.sh (self-heal only
# rewrites the lines in TIER_VARS above, leaving the constant line untouched).
...
try:
    ...
    try:
        ...
        # Constant-line fallback: even if the first off/on didn't write it
        # (an old file, or hand-edited away), every page-load self-heal
        # re-adds MAX_CONTEXT_LINE.
